...
view = st.sidebar.selectbox(
    "Select View:",
    ["🏆 Standings", "🎯 Shot Maps", "📊 Team Analysis", "👤 Player Analysis", "📈 xG Analysis"]
)
...
if view == "🎯 Shot Maps":
    ...
    with col2:    
        if 'Match' in season_shots.columns:
            ...
            matches = sorted(
                season_shots.loc[season_shots['team'] == selected_team, 'Match'].dropna().unique()
            )
            ...
            if len(matches) == 0:
                ...
                match_shots = season_shots[
                    (season_shots['Match'] == selected_match) & 
                    (season_shots['team'] == selected_team)
                ]
    ...
    if not filtered_shots.empty:
        if viz_type == "Shot Map":
            # Original shot map visualization
            ...
            x_coords = filtered_shots['X'].values * 120
            y_coords = (1 - filtered_shots['Y'].values) * 80
            
            # Create KDE heatmap exactly like the reference
            sns.kdeplot(
                data=filtered_shots, 
                x=x_coords, 
                y=y_coords, 
...
# ============================================================================
    ...
    if data['scorers'] is not None:
        ...
        if not season_scorers.empty:
            ...
            st.dataframe(filtered_scorers, use_container_width=True, height=400, hide_index=True)
...
# ============================================================================
    ...
    if data['standings'] is not None:
        ...
        if not season_data.empty:
            ...
            st.dataframe(styled_df, use_container_width=True, height=700, hide_index=True, column_config={
