...
def get_football_data_matches(season):
    ...
    if matches_data and 'matches' in matches_data:
        ...
        # Clean team names - remove FC, AFC
        df['home_team'] = df['home_team'].str.replace(' FC', '', regex=False).str.replace(' AFC', '', regex=False)
        df['away_team'] = df['away_team'].str.replace(' FC', '', regex=False).str.replace(' AFC', '', regex=False)
        
        # Create join key
        df['join_key'] = df['date'] + '_' + df['home_team']
...
# Create merged datasets per season (Football-Data matches + Understat xG)
...
for idx, season in enumerate(SEASONS):
    ...
    if os.path.exists(fd_matches_file) and os.path.exists(us_matches_file):
        ...
        # Merge on join_key (date_home_team)
        merged = pd.merge(
            fd_df,
            us_df[['join_key', 'home_xg', 'away_xg', 'match_id']],
            on='join_key',
            how='left',
            suffixes=('', '_understat')
        )
...
print("\nNext steps:")
print("  1. Use matches_merged_{season}.csv for comprehensive match analysis per season")
print("  2. Use shots_{season}.csv for shot maps and player analysis")
